...
declare function search:CACHE_ALI_RE_Attorney_Data()
{
	...
	let $newcontent := element {"CACHINGDATA"} {
			for $data in $results[1 to 80000]
	...
	let $insert := xdmp:document-insert("/LegalCompass/CACHINGDATA/ALI_RE_Attorney_Data1.xml",$newcontent)
	...
	let $newcontent := element {"CACHINGDATA"} {
			for $data in $results[80001 to 160000]
		let $Name := $data//ALI_RE_Attorney_Data:attorney_name/text()
		(:let $SearchName := fn:replace($Name,'[^a-zA-Z0-9 ]',''):)
		(:let $SearchName := fn:replace($Name,'[^a-zA-Z0-9'']',''):)
		let $SearchName := fn:replace(fn:replace($Name,'[^a-zA-Z0-9'']',''),'[,-?!$*#{}/?:~@%<>|+]','')
	...
	let $insert := xdmp:document-insert("/LegalCompass/CACHINGDATA/ALI_RE_Attorney_Data2.xml",$newcontent)
	
	let $newcontent := element {"CACHINGDATA"} {
			for $data in $results[160001 to 240000]
		let $Name := $data//ALI_RE_Attorney_Data:attorney_name/text()
		(:let $SearchName := fn:replace($Name,'[^a-zA-Z0-9 ]',''):)
		(:let $SearchName := fn:replace($Name,'[^a-zA-Z0-9'']',''):)
		let $SearchName := fn:replace(fn:replace($Name,'[^a-zA-Z0-9'']',''),'[,-?!$*#{}/?:~@%<>|+]','')
	...
	let $insert := xdmp:document-insert("/LegalCompass/CACHINGDATA/ALI_RE_Attorney_Data3.xml",$newcontent)
	
	let $newcontent := element {"CACHINGDATA"} {
			for $data in $results[240001 to last()]
		let $Name := $data//ALI_RE_Attorney_Data:attorney_name/text()
		(:let $SearchName := fn:replace($Name,'[^a-zA-Z0-9 ]',''):)
		(:let $SearchName := fn:replace($Name,'[^a-zA-Z0-9'']',''):)
		let $SearchName := fn:replace(fn:replace($Name,'[^a-zA-Z0-9'']',''),'[,-?!$*#{}/?:~@%<>|+]','')
	...
	let $insert := xdmp:document-insert("/LegalCompass/CACHINGDATA/ALI_RE_Attorney_Data4.xml",$newcontent)
...
(:			let $ALT_Name := cts:search(/TOP500,
 ...
 :)     
  ...
  {
	for $x in cts:search(/SURVEYLISTING,
			cts:and-query((
				 cts:directory-query($config:RD-SURVEYLISTING-PATH)		
...
declare function search:GetQuickSearchResults($term,$type)
{	
