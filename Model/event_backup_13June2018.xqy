...
)
{
								 ...
								 event:GetExactAndOperatorQuery($Keywords)	:)
								   ...
								   cts:or-query((
									...
									cts:element-word-query(xs:QName('ALI_RE_Event_Data:practice_area'),fn:tokenize($Keywords,' '),('wildcarded','case-insensitive'))
										...
										cts:element-word-query(xs:QName('ALI_RE_Event_Data:EventTitle'),$Keywords,('wildcarded','case-insensitive')),
										cts:element-word-query(xs:QName('ALI_RE_Event_Data:EventDescription'),$Keywords,('wildcarded','case-insensitive')),
										cts:element-word-query(xs:QName('ALI_RE_Event_Data:practice_area'),$Keywords,('wildcarded','case-insensitive'))
										))
	...
	return if(fn:contains($response,'[')) then $response else fn:concat('[',$response,']')
};
...
)
{
	...
	let $city_q := if($Cities !='') then
			cts:element-value-query(xs:QName('ALI_RE_Event_Data:city'), fn:tokenize($Cities,'-')[1], ('case-insensitive'))
		else ()
	...
	let $query := cts:or-query((
										...
										for $item in fn:tokenize(fn:lower-case($Keywords),'" or "')
											...
											return 
											 ...
											 cts:element-word-query(xs:QName('ALI_RE_Event_Data:EventTitle'),fn:replace($item,'"',''),('case-insensitive'))
										...
										cts:or-query((for $item in fn:tokenize(fn:lower-case($Keywords),'" or "')
											...
											return 
											 ...
											 cts:element-word-query(xs:QName('ALI_RE_Event_Data:EventDescription'),fn:replace($item,'"',''),('case-insensitive'))
										...
										cts:or-query((for $item in fn:tokenize(fn:lower-case($Keywords),'" or "')
											...
											return 
											 ...
											 cts:element-word-query(xs:QName('ALI_RE_Event_Data:practice_area'),fn:replace($item,'"',''),('case-insensitive'))
										))
